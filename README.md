...
- **MusicAgent** (`server/bot.py`): owns the Pipecat transport and bridges frames to the voice agent. Calls the SDK's `attach_ui_bridge(...)` from `on_ready` so client `ui-event` RTVI messages flow onto the bus as `BusUIEventMessage` and `UIAgent.send_command(...)` delivers `ui-command` messages back to the client.
...
- **SDK UI agent protocol**: `bot.py` calls `attach_ui_bridge(self, target="ui")` from `on_ready`. Client `PipecatClient.sendUIEvent(event, payload)` calls land on the bus as `BusUIEventMessage` and dispatch to `@on_ui_event(name)` handlers on the UI agent. Server-side `send_command(name, payload)` flows the other way through the bridge as an `RTVIUICommandFrame`, which the RTVI observer wraps into a `ui-command` envelope on the wire. Task-lifecycle traffic (`group_started`, `task_update`, etc.) leaves as `RTVIUITaskFrame` and arrives client-side as `ui-task`.
